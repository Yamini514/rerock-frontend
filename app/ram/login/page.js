...
import { ramTeam, demoRamCredentials } from "@/lib/data/staff";

export default function RamLoginPage() {
  const router = useRouter();
  const { toast } = useToast();
  const { user, login } = useRamAuth();
  const [form, setForm] = useState({ email: "", password: "" });
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (user) router.replace("/ram/dashboard");
  }, [user, router]);
  ...
  function handleSubmit(e) {
    ...
    setLoading(true);
    setTimeout(() => {
      const matched = ramTeam.find((r) => r.email.toLowerCase() === form.email.trim().toLowerCase()) || ramTeam[0];
      login(matched);
      ...
      <div className="w-full max-w-4xl">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <span className="mt-4 flex items-center gap-1.5 rounded-pill border border-white/15 bg-white/5 px-3.5 py-1.5 text-xs font-medium text-white/70">
            <ShieldCheck className="h-3.5 w-3.5" /> REROCK Advisory Member Portal
          </span>
        </div>

        <div className="grid gap-6 lg:grid-cols-[1fr_1.1fr] lg:items-start">
          <Card className="p-8">
            <h1 className="font-display text-2xl text-ink">Sign in to your desk</h1>
            <p className="mt-1.5 text-sm text-ink-muted">Manage client portfolios, recommendations, and referrals.</p>
          ...
          <Card className="p-8">
            ...
            <p className="mt-1.5 text-sm text-ink-muted">Tap a REROCK Advisory Member to autofill and sign in as them.</p>

            <div className="mt-6 space-y-3">
              {demoRamCredentials.map((cred) => (
                <button
                  key={cred.id}
                  type="button"
