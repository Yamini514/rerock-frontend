...
export default function AdminPortalLayout({ children }) {
  ...
  useEffect(() => {
    if (user && pathname) pushRecent(pathname);
  }, [user, pathname, pushRecent]);
  ...
  const allowed = canAccessPage(user, pathname);
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 bg-bg pb-20 lg:pb-0">
          {allowed ? (
            children
          ) : (
            <PermissionState
      ...
      <AdminMobileNav onOpenMore={() => setMobileOpen(true)} />
    </div>
  );
}
