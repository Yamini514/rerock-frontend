...
// clientId links to lib/data/clients.js; uploadedBy/uploadedByRole record who attached the file
// (Agent/RAM upload client-facing deal documents, Admin uploads property-level documents — see
// addDocument() below). All four seed rows belong to Kiran Kumar Reddy (c1), the one full-depth
// demo persona, uploaded by his assigned Agent (Priya Reddy) / RAM (Neha Kapoor).
export const documents = [
  { id: "d1", name: "Sale Agreement — Sobha Royal Crest", date: "2022-11-05", type: "PDF", clientId: "c1", propertyId: null, uploadedBy: "Priya Reddy", uploadedByRole: "Agent", url: null },
  { id: "d2", name: "Allotment Letter — Brigade Horizon", date: "2024-03-12", type: "PDF", clientId: "c1", propertyId: null, uploadedBy: "Priya Reddy", uploadedByRole: "Agent", url: null },
  { id: "d3", name: "RERA Certificate — Financial District Office", date: "2021-07-20", type: "PDF", clientId: "c1", propertyId: null, uploadedBy: "Neha Kapoor", uploadedByRole: "RAM", url: null },
  { id: "d4", name: "Payment Receipt — Aparna Zenon", date: "2023-01-30", type: "PDF", clientId: "c1", propertyId: null, uploadedBy: "Neha Kapoor", uploadedByRole: "RAM", url: null },
];

// Mutates `documents` in place (same singleton-mutation pattern as staff.js's updateUserPermissions)
// so a newly uploaded document shows up anywhere else that reads this array in the same session —
// there's no backend yet, so this doesn't survive a hard refresh.
export function addDocument({ name, type, url, size, clientId = null, propertyId = null, uploadedBy, uploadedByRole }) {
  const doc = {
