...
import { agentNotifications } from "@/lib/data/agentNotifications";
import { cn } from "@/lib/utils";

export function AgentTopbar({ onOpenMobile }) {
  ...
  const myNotifications = agentNotifications.filter((n) => n.agentSlug === user?.slug);
    ...
    >
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        >
          ...
          <Link href="/agent/notifications" className="mt-1 block rounded-xl px-3 py-2 text-center text-sm font-medium text-primary hover:bg-surface-soft">
          ...
          trigger={() => (
            ...
            >
              <span className="relative h-8 w-8 overflow-hidden rounded-full bg-primary-soft ring-2 ring-primary/20">
                {user?.avatar && <Image src={user.avatar} alt={user?.name || "Agent"} fill sizes="32px" className="object-cover" />}
              </span>
