...
export function ClientSidebar({ mobileOpen, onCloseMobile }) {
    ...
    return (
      <motion.div whileHover={{ x: 3 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.15, ease: [0.22, 1, 0.36, 1] }}>
        ...
        >
          {active && (
            <motion.span
              layoutId={`sidebar-active-${scope}-${layoutId}`}
      ...
      {user?.name && (
        <div className="mx-4 mb-4 flex items-center gap-3 rounded-2xl bg-surface-soft p-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-ink-faint">Investor · Member since {user.memberSince}</p>
          </div>
        </div>
      )}

      <nav className="flex-1 space-y-1 overflow-y-auto px-4 pb-8">
        {clientNav.map((group, i) => (
