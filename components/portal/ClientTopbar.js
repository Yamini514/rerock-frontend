...
export function ClientTopbar({ onOpenMobile }) {
    ...
    >
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        >
          ...
          <Link href="/portal/notifications" className="mt-1 block rounded-xl px-3 py-2 text-center text-sm font-medium text-primary hover:bg-surface-soft">
          ...
          trigger={() => (
            ...
            >
              <span className="relative h-8 w-8 overflow-hidden rounded-full bg-primary-soft ring-2 ring-primary/20">
                {user?.avatar && <Image src={user.avatar} alt={user?.name || "Client"} fill sizes="32px" className="object-cover" />}
              </span>
